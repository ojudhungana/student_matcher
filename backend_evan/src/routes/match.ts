...
async function getMyTags(userId: string): Promise<string[]> {
  const { data } = await supabaseService
...
function incompleteReasons(p: any): string[] {
  const reasons: string[] = [];
  if (!p) { reasons.push('no profile'); return reasons; }
  if (!p.name) reasons.push('name');
  if (!p.birthday) reasons.push('birthday');
  if (!p.major) reasons.push('major');
  if (!p.year) reasons.push('year');
  if (!p.commuter_status) reasons.push('commuter_status');
  if (!p.bio || typeof p.bio !== 'string' || p.bio.trim().length < 20 || p.bio.trim().length > 200) {
    reasons.push('bio (20–200 chars)');
  }
  if (!Array.isArray(p.photos) || p.photos.length < 5) reasons.push('at least 5 photos');
  if (Array.isArray(p.photos) && p.photos.length > 10) reasons.push('max 10 photos');
  return reasons;
}
const isComplete = (p: any) => incompleteReasons(p).length === 0;
...
router.get('/next', requireAuth, async (req, res) => {
  ...
  const excludeIds = new Set<string>([me.id, ...(already?.map(r => r.target_id) || [])]);
...
router.post('/like', requireAuth, async (req, res) => {
  ...
  const myProfile = await getMyProfile(me.id);
  const reasons = incompleteReasons(myProfile);
  if (reasons.length) return res.status(400).json({ error: 'Complete your profile first.', missing: reasons });
